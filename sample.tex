...
\begin{frame}{文法判断能力(BLiMP)の評価・考察}
    \begin{table}[htbp]
    \caption{$\rho(W^{\mathrm{rec}})$ と$D_{\mathrm{emb}}$の変化に伴うBLiMP精度}
    \centering
    \small
    \tabcolsep=4pt
    \begin{tabular}{l | cccccc}
        \hline
        スペクトル半径 $\rho(W^{\mathrm{rec}})$ & 0.80 & 0.85 & 0.90 & 0.95 & 1.00 & 1.05 \\
        \hline
        次元数 $D_{emb}$ & \multicolumn{6}{c}{精度 (Macro Average)} \\
        \hline\hline
        32  & 55.68\% & 55.57\% & 55.48\% & 55.49\% & ${\color{RoyalBlue}\underline{\mathbf{55.50\%}}}$ & ${\color{RoyalBlue}\underline{\mathbf{55.53\%}}}$ \\
        64  & ${\color{RoyalBlue}\underline{\mathbf{56.05\%}}}$ & ${\color{RoyalBlue}\underline{\mathbf{55.97\%}}}$ & ${\color{RoyalBlue}\underline{\mathbf{55.77\%}}}$ & ${\color{RoyalBlue}\underline{\mathbf{55.51\%}}}$ & 55.26\% & 54.93\%\\
...
\section{まとめ・議論}
\begin{frame}{まとめ}
    \begin{block}{一般の時系列処理を行うRCと別の挙動を示す}
        Sample text
    \end{block}

    \begin{alertblock}{入力するベクトルの次元数は挙げ続けても性能は上がらない}
        Sample text in red box
    \end{alertblock}

    \begin{example}
        次元数32でスペクトル半径が1以上のモデルは計算量を大幅に落としながらも性能を落としづらい(頑健性を持っている)
    \end{example}
\end{frame}
